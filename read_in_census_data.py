...
class CensusInfo (object):
    ...
    variable_properties_dict   = \
          dict(list(zip(variable_properties_list,variable_properties_cols)))
    ...
    for file_type in list(variable_explanation_cols_dict.keys()):
        variable_explanation_cols_dict[file_type] = \
                         dict(list(zip(variable_explanation_list,variable_explanation_cols_dict[file_type])))
            ...
            for row_num in range(thesheet.nrows):
                ...
                if this_var and any((x.strip() for x in row)):
                    ...
                    for file_type in self.file_types:
                        ...
                        (low_str, hi_str, desc) = (row[these_cols['LO']],row[these_cols['HI']],
    ...
    def print_var_info (self, var0, record_type, file_type=None):
        ...
        var_values = sorted(var_values_info.keys(),key=lambda x:''.join([str(y) for y in x]))
